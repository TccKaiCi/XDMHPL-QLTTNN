USE master
DROP DATABASE TrungTamNgoaiNguDB
CREATE DATABASE TrungTamNgoaiNguDB
USE TrungTamNgoaiNguDB

CREATE TABLE THISINH (
	CMND NVARCHAR(50) NOT NULL,
	HoTen NVARCHAR(50) NOT NULL,
	GioiTinh NVARCHAR(50) NOT NULL,
	NgaySinh datetime NOT NULL,
	NoiSinh NVARCHAR(50) NOT NULL,
	SDT int NOT NULL
)

CREATE TABLE PHONGTHI (
	MaPhongThi NVARCHAR(50) NOT NULL,
	TenPhongThi NVARCHAR(50) NOT NULL,
	MaKhoaThi NVARCHAR(50) NOT NULL,
	SoLuong int NOT NULL
)

CREATE TABLE PHIEUDUTHI (
	SoBaoDanh NVARCHAR(50) NOT NULL,
	MaPhongThi NVARCHAR(50) NOT NULL,
	CMND NVARCHAR(50) NOT NULL,
	MaKhoaThi NVARCHAR(50) NOT NULL
)

CREATE TABLE KHOATHI (
	MaKhoaThi NVARCHAR(50) NOT NULL,
	TenKhoaThi NVARCHAR(50) NOT NULL,
	TrinhDo NVARCHAR(50) NOT NULL,
	CaThi NVARCHAR(50) NOT NULL,
	NgayThi datetime NOT NULL
)

CREATE TABLE KETQUA (
	SoBaoDanh NVARCHAR(50) NOT NULL,
	MaKhoaThi NVARCHAR(50) NOT NULL,
	DiemNghe int NOT NULL,
	DiemNoi int NOT NULL,
	DiemDoc int NOT NULL,
	DiemViet int NOT NULL
)

CREATE TABLE DIADIEM (
	MADIADIEM NVARCHAR(50) NOT NULL,
	TENDIADIEM NVARCHAR(50) NOT NULL
)

CREATE TABLE GIOITINH (
	GIOITINH NVARCHAR(50) NOT NULL
)


--KEy -----------------------------------------------------------------
-- PK
ALTER TABLE THISINH
  ADD CONSTRAINT PK_CMND PRIMARY KEY (CMND);

ALTER TABLE PHONGTHI
  ADD CONSTRAINT PK_TenPhongThi PRIMARY KEY (MaPhongThi);

ALTER TABLE KHOATHI
  ADD CONSTRAINT PK_MaKhoaThi PRIMARY KEY (MaKhoaThi);

ALTER TABLE DIADIEM
  ADD CONSTRAINT PK_MADIADIEM PRIMARY KEY (MADIADIEM);

ALTER TABLE GIOITINH
  ADD CONSTRAINT PK_GIOITINH PRIMARY KEY (GIOITINH);

ALTER TABLE PHIEUDUTHI
  ADD CONSTRAINT PK_SOBAODANH PRIMARY KEY (SOBAODANH);

-- FK
ALTER TABLE PHONGTHI
  ADD CONSTRAINT FK_PHONGTHI_MAKHOATHI FOREIGN KEY (MaKhoaThi) REFERENCES KHOATHI (MaKhoaThi);

ALTER TABLE THISINH
  ADD CONSTRAINT FK_THISINH_MADIADIEM FOREIGN KEY (NoiSinh) REFERENCES DIADIEM (MADIADIEM);

ALTER TABLE THISINH
  ADD CONSTRAINT FK_THISINH_GIOITINH FOREIGN KEY (GioiTinh) REFERENCES GIOITINH (GIOITINH);


ALTER TABLE PHIEUDUTHI
  ADD CONSTRAINT FK_PHIEUDUTHI_TenPhongThi FOREIGN KEY (MaPhongThi) REFERENCES PHONGTHI (MaPhongThi);
ALTER TABLE PHIEUDUTHI
  ADD CONSTRAINT FK_PHIEUDUTHI_CMND FOREIGN KEY (CMND) REFERENCES THISINH (CMND);
ALTER TABLE PHIEUDUTHI
  ADD CONSTRAINT FK_PHIEUDUTHI_MaKhoaThi FOREIGN KEY (MaKhoaThi) REFERENCES KHOATHI (MaKhoaThi);

ALTER TABLE KETQUA
  ADD CONSTRAINT FK_KQUA_SoBaoDanh FOREIGN KEY (SoBaoDanh) REFERENCES PHIEUDUTHI (SoBaoDanh);
ALTER TABLE KETQUA
  ADD CONSTRAINT FK_KQUA_MaKhoaThi FOREIGN KEY (MaKhoaThi) REFERENCES KHOATHI (MaKhoaThi);

-- Super PK
ALTER TABLE KETQUA
  ADD CONSTRAINT PKMASV_MAKH PRIMARY KEY (SoBaoDanh, MaKhoaThi);



INSERT INTO DIADIEM VALUES 
('DD1', 'TPHCM'),
('DD2', 'Ha Noi'),
('DD3', 'Ca Mau'),
('DD4', 'Can Tho'),
('DD5', 'Thanh Haa'),
('DD6', 'Đa Nang'),
('DD7', 'Nha Trang'),
('DD8', 'Vung Tau'),
('DD9', 'Lang Son'),
('DD10', 'Thai Binh');

INSERT INTO GIOITINH VALUES
('Nam'),
('Nu'),
('Khac');


INSERT INTO THISINH VALUES 
('123456789','Tang Chi Chung', 'Nam', '2022-01-04 00:00:00.000', 'DD1', '0123456789'),
('987654321','Nguyen Tuan Anh', 'Nam', '2022-01-04 00:00:00.000', 'DD3', '0987654321'),
('121212121','Nguyen Thien Tam', 'Nam', '2022-01-04 00:00:00.000', 'DD5', '0121212121'),
('232323232','Do Thi Linh', 'Nu', '2022-01-04 00:00:00.000', 'DD7', '0232323232');

INSERT INTO KHOATHI VALUES 
('K18_TT_A2','Khoa 18 A2', 'A2', 'Ca sáng', '2022-01-04 00:00:00.000'),
('K18_TTN_B1','Khoa 17 B1', 'B1','Ca sáng', '2022-01-04 00:00:00.000');

INSERT INTO PHONGTHI VALUES
('P123456','A2P01', 'K18_TT_A2', '30'),
('P654321','B1P01', 'K18_TTN_B1', '30');

INSERT INTO PHIEUDUTHI VALUES
('A2001' ,'P123456','123456789' , 'K18_TT_A2'),
('A2002' ,'P123456','987654321' , 'K18_TT_A2'),
('A2003' ,'P123456','121212121' , 'K18_TT_A2'),
('B1001' ,'P654321','232323232' , 'K18_TTN_B1');

INSERT INTO KETQUA VALUES
('A2001', 'K18_TT_A2', '7', '7', '7', '7'),
('A2002', 'K18_TT_A2', '7', '9', '9', '7' ),
('A2003', 'K18_TT_A2', '7', '7', '7', '7' ),
('B1001', 'K18_TTN_B1', '7', '7', '7', '7');




select * from ketqua
select * from khoathi
select * from phieuduthi
select * from phongthi
select * from thisinh
select * from DIADIEM


select * from ketqua
select * from thisinh
select * from phieuduthi

select * from thisinh, phieuduthi where thisinh.CMND = PHIEUDUTHI.CMND and THISINH.HoTen = 'Tang CHUNG'